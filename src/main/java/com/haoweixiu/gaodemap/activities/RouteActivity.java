...
import com.haoweixiu.gaodemap.adapter.BusResultListAdapter;
import com.haoweixiu.gaodemap.route.DriveRouteColorfulOverLay;
import com.haoweixiu.gaodemap.util.AMapUtil;
import com.haoweixiu.gaodemap.util.ToastUtil;
...
OnMarkerClickListener, OnInfoWindowClickListener, InfoWindowAdapter, OnRouteSearchListener,LocationSource,
	...
	private void init() {
		...
		registerListener();
		int i_lat= address.indexOf("@#");
		int i_addreess = address.lastIndexOf("@#");
		String str_lat = address.substring(0,i_lat);
		String str_lng = address.substring(i_lat+2,i_addreess);
		double dlat = Double.parseDouble(str_lat);
		double dlng = Double.parseDouble(str_lng);
		mEndPoint = new LatLonPoint(dlat, dlng);
